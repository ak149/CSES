...
void FloydWarshall(){
	distances = vector<vector<ll>>(n + 1,vector<ll>(n + 1,INF));
	fori(i,1,n + 1){
		fori(j,1,n + 1){
			distances[i][j] = g[i][j];
		}
...
void solve(){
	...
	g = vector<vector<ll>>(n + 1,vector<ll>(n + 1,INF));

	while(m--){
		ll u,v,weight;
		cin>>u>>v>>weight;

		g[u][v] = min(weight,g[u][v]);
		g[v][u] = min(weight,g[u][v]);
	}
	...
	while(Q--){
		...
		if(distances[sv][ev] == INF){
			cout<<-1<<endl;
		}
		else{
			cout<<distances[sv][ev]<<endl;
		}
	}

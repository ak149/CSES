...
void solve() {
	...
	for (ll i = n - 1; i >= 0 ; i--) {
		...
		while (j < k) {
			if (sorted[i].fi + sorted[j].fi + sorted[k].fi == target) {
				vll idx = {sorted[i].se, sorted[j].se, sorted[k].se};
				sort(all(idx));
				for (auto id : idx)	cout << id + 1 << " ";
				return;
			}
			else if (sorted[i].fi + sorted[j].fi + sorted[k].fi < target) {
				j++;
			}
			else {
				k--;
